...
describe APIv1::Withdraw, type: :request do
  ...
  def check_access_headers(response)
    expect(response.headers['Access-Control-Allow-Headers'])
      .to eq('Origin, X-Requested-With, Content-Type, Accept, Authorization')
    expect(response.headers['Access-Control-Allow-Credentials']).to eq('false')
  end

  def check_acess_methods(response)
  ...
  it 'sends CORS headers when requesting using OPTIONS' do
    ...
    check_access_headers(response)
    check_acess_methods(response)
  end
